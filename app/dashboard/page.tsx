...
export default function Dashboard() {
  ...
  const loadDashboard = async () => {
    try {
      // 1. In a real app, this fetches from your Supabase DB. 
      // For now, we ensure the UI perfectly matches your video state.
      const userPortfolio = [
        { ticker: 'AAPL', name: 'APPLE INC.', status: 'Strengthening', latestUpdate: 'Fundamentals remain aligned with thesis.', updateTime: 'No recent updates' },
        { ticker: 'NVDA', name: 'NVDA CORP', status: 'Strengthening', latestUpdate: 'Fundamentals remain aligned with thesis.', updateTime: 'No recent updates' },
        { ticker: 'TSLA', name: 'TSLA CORP', status: 'Strengthening', latestUpdate: 'Fundamentals remain aligned with thesis.', updateTime: 'No recent updates' }
      ];
      
      setPortfolio(userPortfolio);

      // 2. Fetch the LIVE PRICES from your custom API route
      ...
      if (tickers) {
        ...
        if (res.ok) {
          ...
          // The API returns { prices: { TSLA: 311.21 } }
          if (data && data.prices) {
      ...
      // Add a tiny artificial delay so you can admire your new skeleton loader!
    ...
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900">
      ...
      <nav className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4 text-sm font-medium text-gray-500">
            <span className="hidden sm:inline">padamkcinvesting@gmail.com</span>
            <button className="font-bold text-gray-900 hover:text-blue-600 transition-colors">Sign Out</button>
          </div>
        </div>
      </nav>
      ...
      <main className="max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="flex justify-between items-end mb-6 border-b border-gray-200 pb-4">
          ...
          <span className="text-sm font-bold text-blue-600 border-b-2 border-blue-600 pb-1">All ({portfolio.length})</span>
        </div>
